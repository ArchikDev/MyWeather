...
import com.example.myweather.domain.usecase.GetCurrentWeatherUseCase
import com.example.myweather.domain.usecase.GetFavouriteCitiesUseCase
import com.example.myweather.presentation.favourite.FavouriteStore.Intent
import com.example.myweather.presentation.favourite.FavouriteStore.Label
import com.example.myweather.presentation.favourite.FavouriteStore.State
...
class FavouriteStoreFactory @Inject constructor(
    ...
    // Когда state меняется из репозитория , а не user
    private sealed interface Action {

        data class FavouriteCitiesLoaded(val cities: List<City>): Action
    }

    // Изменения state на экране
    ...
    private object ReducerImpl : Reducer<State, Msg> {
        override fun State.reduce(msg: Msg): State = when(msg) {
            ...
            is Msg.WeatherLoaded -> {
                copy(cityItems = cityItems.map {
                    if(it.city.id == msg.cityId) {
                        it.copy(weatherState = State.WeatherState.Loaded(msg.tempC, msg.conditionIconUrl))
                    } else {
                        it
                    }
                })
